...
The canonical documentation version is `aoxc.v.0.0.0-alpha.3` and the Cargo-compatible semantic version is `0.0.0-alpha.3`. The repository treats documentation, manifests, binaries, compatibility fixtures, and verification evidence as a single release package. A material change is expected to update the relevant version and ledger documentation together.
...
## Security Audit Log
- `aoxc.v.0.0.0-alpha.3`: root README expanded into a full system-level guide with subsystem mapping, command documentation, operational entry points, and release-governance context.
