...
pub fn build_block_validation_report_with_locale(
    ...
    let mut events = vec![ValidationEvent {
        ...
        title: localized(locale, "Validation started", "Doğrulama başlatıldı"),
        message: localized(
            locale,
            "The block is being validated against protocol rules.",
            "Blok protokol kurallarına göre doğrulanıyor.",
        ),
        action: localized(locale, "Wait for completion.", "Tamamlanmasını bekleyin."),
    }];
    ...
    match result {
        Ok(()) => {
            events.push(ValidationEvent {
                ...
                title: localized(locale, "Block accepted", "Blok kabul edildi"),
                message: localized(
                    locale,
        ...
        Err(err) => {
            let desc = describe_block_error_with_locale(err, locale);
...
pub fn build_validation_envelope_with_locale(
    ...
    let report = build_block_validation_report_with_locale(block, locale);
