...
mod tests {
    ...
    #[test]
    fn kernel_operation_rejects_unauthorized_role() {
        let policy = NodePolicy::for_network_class(NetworkClass::PublicMainnet);
        let eligible_signers: HashSet<String> = [
            "quorum-1".to_string(),
            "quorum-2".to_string(),
        ...
        assert!(matches!(
            err,
            GenesisConfigError::InvalidKernelOperationRole { .. }
        ));
    }

    #[test]
    fn kernel_operation_rejects_seal_commitment_without_active_quantum_layer() {
        let mut policy = NodePolicy::for_network_class(NetworkClass::PublicTestnet);
        policy.seal_layers = vec![SealLayerPolicy {
            layer_id: "seal-v1".into(),
            commitment_hash: "BLAKE3".into(),
        ...
        let err = policy
            .enforce_kernel_operation(
                NodeRole::Seal,
                KernelOperation::SealCommitment,
                0,
                &[
                    "seal-1".into(),
                    "seal-2".into(),
        ...
        assert!(matches!(
            err,
            GenesisConfigError::MissingActiveQuantumSealLayer { .. }
        ));
    }
}
