...
## Executive Summary
AOXCAI models AI-assisted governance, capability control, manifest policy, and backend integration for advisory features within AOX Chain.
...
## Main Code Areas
- `manifest`, `audit`, `capability`, `constitution`, and `policy`: the governance boundary for AI-assisted actions.
- `backend`: backend factory, heuristic logic, and remote HTTP model integration.

## Security and Audit Focus
Capability boundaries, remote backend policy, and compatibility declarations must remain explicit and reviewable.
