...
mod tests {
    ...
    #[tokio::test]
    async fn infer_succeeds_with_bearer_auth_when_environment_variable_is_present() {
        let server = MockServer::start();
        let mock = server.mock(|when, then| {
            when.method(POST)
                .path("/infer")
        ...
        let env_key = "AOXC_TEST_REMOTE_HTTP_TOKEN_OK_37B5D2F4";
        unsafe { std::env::set_var(env_key, "super-secret-token") };

        let mut manifest =
            bearer_remote_http_manifest(format!("{}/infer", server.base_url()), env_key);
        manifest.spec.security.allow_private_networks = true;
        manifest.spec.security.allowed_endpoints = vec![server.base_url()];
        ...
        let output = backend
            .infer(&manifest, &empty_request())
            .await
            .expect("bearer auth request must succeed");
        mock.assert();
    ...
    #[tokio::test]
    async fn infer_includes_configured_custom_headers() {
        let server = MockServer::start();
        let mock = server.mock(|when, then| {
            when.method(POST)
                .path("/infer")
                .header("x-aoxc-trace", "trace-test-header");
            then.status(200)
                .header("content-type", "application/json")
                .json_body(json!({
                    "backend": "remote_http",
        ...
        manifest
            ...
            .headers
            .insert("X-AOXC-Trace".to_owned(), "trace-test-header".to_owned());
        ...
        let output = backend
            .infer(&manifest, &empty_request())
            .await
            .expect("request with custom headers must succeed");
        mock.assert();

        assert_eq!(output.risk_bps, 777);
    }
    ...
    #[tokio::test]
    async fn infer_with_zero_retries_attempts_once_and_returns_error() {
        let server = MockServer::start();
        let failure = server.mock(|when, then| {
            when.method(POST).path("/infer");
            then.status(500).body("single-attempt failure");
        });

        let mut manifest = remote_http_manifest(format!("{}/infer", server.base_url()));
        manifest.spec.security.allow_private_networks = true;
        manifest.spec.security.allowed_endpoints = vec![server.base_url()];
        manifest.spec.backend.max_retries = 0;
        let backend = RemoteHttpBackendRuntime::new(&manifest).expect("backend must construct");

        let err = backend
            .infer(&manifest, &empty_request())
            .await
            .expect_err("single attempt must return error");

        assert!(matches!(err, AiError::BackendFailure(_)));
        assert_eq!(failure.calls(), 1);
    }

    #[tokio::test]
    async fn infer_classifies_timeouts_as_backend_timeout() {
        use std::io::Read;
        use std::net::TcpListener;
        ...
        manifest.spec.security.allow_private_networks = true;
        manifest.spec.security.allowed_endpoints = vec![base_url];
        manifest.spec.backend.timeout_ms = 50;
        ...
        server.join().expect("timeout server thread must complete");

        assert!(matches!(err, AiError::BackendTimeout(_)));
    }

    #[tokio::test]
    async fn infer_maps_generic_reqwest_errors_to_http() {
        let server = MockServer::start();
        let mut manifest = remote_http_manifest(format!("{}/infer", server.base_url()));
        manifest.spec.security.allow_private_networks = true;
        manifest.spec.security.allowed_endpoints = vec![server.base_url()];
        ...
        manifest
            ...
            .expect("remote http config must exist")
            .headers
            .insert("X-Invalid".to_owned(), "bad\nvalue".to_owned());
        let backend = RemoteHttpBackendRuntime::new(&manifest).expect("backend must construct");

        let err = backend
            .infer(&manifest, &empty_request())
            .await
            .expect_err("redirect loop must surface as HTTP error");
