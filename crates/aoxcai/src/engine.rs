...
mod tests {
    ...
    #[tokio::test]
    async fn integration_style_engine_remote_backend_success_path() {
        let server = MockServer::start();
        let mock = server.mock(|when, then| {
            when.method(POST).path("/infer");
            then.status(200)
                ...
                .json_body(json!({
                    "backend": "remote_http",
                    "model_id": "validator-risk-v2",
                    "label": "review",
                    "risk_bps": 2500,
                    "confidence_bps": 8200,
                    "rationale": "integration success path",
                    "recommended_action": "review",
                    "attributes": {"trace_id": "it-success"}
                }));
        });
        ...
        let report = engine
            ...
            .await
            .expect("integration-style success path must pass");
        mock.assert();

        assert_eq!(report.model_output.backend, "remote_http");
        assert_eq!(report.assessment.effective_risk_bps, 2500);
        assert_eq!(report.request.context.subject_id, "validator-it-ok");
    }

    #[tokio::test]
    async fn integration_style_engine_fallback_end_to_end() {
        let server = MockServer::start();
        let mock = server.mock(|when, then| {
            when.method(POST).path("/infer");
            then.status(500).body("integration failure");
        });
        ...
        manifest.spec.security.allowed_endpoints = vec![server.base_url()];
        manifest.spec.fallback.enabled = true;
        manifest.spec.fallback.action_on_backend_error = BackendFailureAction::Review;
        ...
        let report = engine
            .evaluate(
                AiTask::ValidatorAdmission,
                AiMode::Enforced,
                "validator-it-fallback",
            )
            .await
            .expect("fallback integration path must return report");
        mock.assert();

        assert_eq!(report.model_output.backend, "fallback");
        assert_eq!(report.assessment.action, DecisionAction::Review);
        assert!(
            report
                .assessment
                .rationale
                .contains("Fallback assessment applied")
        );
    }

    #[tokio::test]
    async fn integration_style_multi_provider_policy_manifest_wiring_consistency() {
        let mut manifest = base_manifest();
        manifest.metadata.id = "multi-provider-wiring-manifest".to_owned();
        manifest.spec.input.max_signal_count = 4;
        ...
        let engine = AiEngine::new(
            ...
            vec![
                Box::new(StaticSignalProvider {
                    signals: vec![
                        InferenceSignal::new("identity", "revoked_identity", 9100, "provider-a"),
                        InferenceSignal::new("runtime", "anomaly", 2500, "provider-a"),
                    ],
                }),
                Box::new(StaticSignalProvider {
                    signals: vec![InferenceSignal::new(
        ...
        let report = engine
            ...
            .await
            .expect("wiring consistency test must pass");

        assert_eq!(report.manifest_id, "multi-provider-wiring-manifest");
        assert_eq!(report.request.signals.len(), 3);
        assert_eq!(report.request.findings.len(), 3);
        assert_eq!(report.assessment.action, DecisionAction::Deny);
        assert_eq!(report.backend_type, "Heuristic");
    }
