...
mod tests {
    ...
    #[test]
    fn from_yaml_file_loads_and_validates_a_known_good_manifest() {
        let manifest = base_manifest();
        let yaml = serde_yaml::to_string(&manifest).expect("manifest must serialize");
        let path = write_temp_manifest(&yaml);

        let parsed =
            ModelManifest::from_yaml_file(&path).expect("serialized base manifest must load");
        assert_eq!(parsed.id(), manifest.id());
        assert_eq!(parsed.spec.backend.r#type, BackendType::Heuristic);
        assert_eq!(parsed.spec.bindings.default_for_tasks.len(), 1);

        fs::remove_file(path).expect("temp manifest should be removable");
    }

    #[test]
    fn validation_rejects_remote_http_none_auth_with_non_empty_env_key() {
        let mut manifest = base_manifest();
        manifest.spec.backend.r#type = BackendType::RemoteHttp;
        manifest.spec.backend.heuristic = None;
        manifest.spec.backend.remote_http = Some(RemoteHttpBackend {
            ...
            auth: Auth {
                mode: AuthMode::None,
                env_key: "SHOULD_BE_EMPTY".to_owned(),
            },
            tls: Tls {
                enabled: true,
                verify_peer: true,
        ...
        let err = manifest
            .validate()
            .expect_err("manifest must fail when auth mode none has env key");
        assert!(matches!(err, AiError::ManifestValidation(_)));
    }

    #[test]
    fn validation_rejects_signature_requirement_without_public_key_or_signature_field() {
        let mut manifest = base_manifest();
        manifest.spec.security.manifest_signature.required = true;
        manifest.spec.security.manifest_signature.public_key_path = None;
        manifest.spec.security.manifest_signature.signature_field = "signature".to_owned();
        let err = manifest
            .validate()
            .expect_err("required signature without public key path must fail");
        assert!(matches!(err, AiError::ManifestValidation(_)));

        manifest.spec.security.manifest_signature.public_key_path =
            Some("/keys/aoxc-manifest.pub".to_owned());
        manifest.spec.security.manifest_signature.signature_field = "   ".to_owned();
        let err = manifest
            .validate()
            .expect_err("required signature with empty signature field must fail");
        assert!(matches!(err, AiError::ManifestValidation(_)));
    }

    #[test]
    fn validation_rejects_remote_http_backend_without_allowed_endpoints() {
        let mut manifest = base_manifest();
        manifest.spec.backend.r#type = BackendType::RemoteHttp;
        manifest.spec.backend.heuristic = None;
        manifest.spec.backend.remote_http = Some(RemoteHttpBackend {
        ...
        manifest.spec.security.allowed_endpoints.clear();

        let err = manifest
            .validate()
            .expect_err("remote http backend without allowed endpoints must fail");
        assert!(matches!(err, AiError::ManifestValidation(_)));
    }

    #[test]
    fn validation_rejects_bindings_outside_supported_tasks_and_empty_supported_modes() {
        let mut manifest = base_manifest();
        manifest.spec.bindings.default_for_tasks = vec![AiTask::ArtifactInspection];
        manifest.spec.compatibility.supported_tasks = vec![AiTask::ValidatorAdmission];

        let err = manifest
            .validate()
            .expect_err("binding outside supported_tasks must fail");
        assert!(matches!(err, AiError::ManifestValidation(_)));

        manifest.spec.bindings.default_for_tasks = vec![AiTask::ValidatorAdmission];
        manifest.spec.compatibility.supported_tasks = vec![AiTask::ValidatorAdmission];
        manifest.spec.compatibility.supported_modes.clear();
        let err = manifest
            .validate()
            .expect_err("empty supported_modes must fail");
        assert!(matches!(err, AiError::ManifestValidation(_)));
    }
}
