...
impl ExecutionLane for DeterministicLane {
    ...
    fn execute(
        ...
        let nonce_key = crate::engine::sender_nonce_key(payload.sender, payload.nonce);
        ...
        let state_key = crate::engine::state_key(&payload.lane_id, &payload.sender, payload.nonce);
...
pub struct DeterministicOrchestrator {
    pub(crate) lane_registry: LaneRegistry,
    pub(crate) lanes: BTreeMap<String, Box<dyn ExecutionLane + Send + Sync>>,
    pub(crate) initial_state: InMemoryStateStore,
}

impl Default for DeterministicOrchestrator {
    fn default() -> Self {
        Self::new(
            crate::default_lane_registry(),
            crate::default_lanes(),
...
mod tests {
    ...
    fn registry_checksum_mismatch_halts_execution() {
        let mut registry = crate::default_lane_registry();
