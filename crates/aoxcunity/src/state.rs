...
impl ConsensusState {
    ...
    /// Builds an execution certificate from the current commit quorum for a block.
    #[must_use]
    ...
    ) -> Option<ExecutionCertificate> {
        let quorum_certificate = self.build_quorum_certificate(block_hash, finalized_round)?;
        Some(ExecutionCertificate::new(
            epoch,
            validator_set_hash,
            quorum_certificate,
        ))
    ...
    /// Attempts to compose a constitutional seal from execution, legitimacy,
    /// and continuity artifacts without mutating the execution-finalized state.
    pub fn try_compose_constitutional_seal(
        &self,
        block_hash: [u8; 32],
        finalized_round: u64,
        epoch: u64,
    ...
    ) -> Result<ConstitutionalSeal, ConsensusError> {
        let execution = self
            .build_execution_certificate(block_hash, finalized_round, epoch, validator_set_hash)
            .ok_or(ConsensusError::InvalidConstitutionalArtifact)?;

        ConstitutionalSeal::compose(&execution, legitimacy, continuity)
            .map_err(|_| ConsensusError::InvalidConstitutionalArtifact)
    }
...
mod tests {
    ...
    #[test]
    fn builds_execution_certificate_from_finalization_path() {
        let mut state = state_with_validators(vec![
            validator(1, 1, ValidatorRole::Validator, true),
            validator(2, 1, ValidatorRole::Validator, true),
            validator(3, 1, ValidatorRole::Validator, true),
        ...
        let execution = state
            .build_execution_certificate(block.hash, 0, 9, [7u8; 32])
            .expect("execution certificate should build from quorum");
        assert_eq!(execution.block_hash, block.hash);
        assert_eq!(execution.epoch, 9);
        assert_eq!(execution.validator_set_hash, [7u8; 32]);
        assert!(execution.validate().is_ok());
    }

    #[test]
    fn composes_constitutional_seal_from_consensus_state_and_artifacts() {
        let mut state = state_with_validators(vec![
            validator(1, 1, ValidatorRole::Validator, true),
            validator(2, 1, ValidatorRole::Validator, true),
            validator(3, 1, ValidatorRole::Validator, true),
        ...
        let continuity =
            ContinuityCertificate::new(block.hash, 0, 0, 9, 0, 2, vec![[1u8; 32], [2u8; 32]]);

        let seal = state
            .try_compose_constitutional_seal(block.hash, 0, 9, [7u8; 32], &legitimacy, &continuity)
            .expect("constitutional seal should compose");
        assert!(
    ...
    #[test]
    fn rejects_constitutional_seal_composition_when_artifacts_do_not_match() {
        let mut state = state_with_validators(vec![
            validator(1, 1, ValidatorRole::Validator, true),
            validator(2, 1, ValidatorRole::Validator, true),
            validator(3, 1, ValidatorRole::Validator, true),
        ...
        let continuity =
            ContinuityCertificate::new([9u8; 32], 0, 0, 9, 0, 2, vec![[1u8; 32], [2u8; 32]]);
        ...
        assert_eq!(
            err.to_string(),
            ConsensusError::InvalidConstitutionalArtifact.to_string()
        );
    }
}
