...
mod tests {
    ...
    #[test]
    fn decode_hex_accepts_prefixed_payload() {
        assert_eq!(decode_hex("0x00ff").expect("valid hex"), vec![0x00, 0xff]);
    }

    #[test]
    fn decode_hex_accepts_non_prefixed_payload() {
        assert_eq!(decode_hex("00ff").expect("valid hex"), vec![0x00, 0xff]);
    }

    #[test]
    fn decode_hex_rejects_odd_length() {
        let err = decode_hex("0x123").expect_err("odd length must fail");
        assert!(err.contains("çift"));
    }

    #[test]
    fn decode_hex_rejects_invalid_chars() {
        let err = decode_hex("0x12zz").expect_err("invalid chars must fail");
        assert!(err.contains("hex parse hatası"));
    }
}
