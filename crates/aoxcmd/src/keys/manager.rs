use crate::{
    error::AppError,
    keys::{loader::{load_operator_key, persist_operator_key}, material::KeyMaterial},
};

pub fn bootstrap_operator_key(name: &str, profile: &str, password: &str) -> Result<KeyMaterial, AppError> {
...
pub fn verify_operator_key() -> Result<(), AppError> {
    ...
    if key.name.trim().is_empty() || key.public_key.trim().is_empty() || key.fingerprint.trim().is_empty() {
