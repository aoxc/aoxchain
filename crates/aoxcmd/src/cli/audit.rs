...
pub fn cmd_diagnostics_bundle(args: &[String]) -> Result<(), AppError> {
    ...
    let bundle_dir = home.join("support").join(format!(
        "bundle-{}",
    ...
    std::fs::create_dir_all(&bundle_dir).map_err(|e| {
        AppError::with_source(
            ...
            format!("Failed to create support bundle directory {}", bundle_dir.display()),
            e,
        )
    })?;
    let report_content = serde_json::to_string_pretty(&report)
        .map_err(|e| AppError::with_source(ErrorCode::OutputEncodingFailed, "Failed to encode diagnostics report", e))?;
    write_file(&bundle_dir.join("doctor.json"), &report_content)?;

    for (source, target) in [
        (home.join("config").join("settings.json"), bundle_dir.join("settings.json")),
        (home.join("identity").join("genesis.json"), bundle_dir.join("genesis.json")),
        (home.join("runtime").join("node_state.json"), bundle_dir.join("node_state.json")),
        (home.join("ledger").join("ledger.json"), bundle_dir.join("ledger.json")),
    ] {
...
pub fn cmd_production_audit(args: &[String]) -> Result<(), AppError> {
    ...
    if let Some(path) = out {
        let content = serde_json::to_string_pretty(&report)
            .map_err(|e| AppError::with_source(ErrorCode::OutputEncodingFailed, "Failed to encode production audit report", e))?;
