...
pub fn run_cli() -> Result<(), AppError> {
    ...
    match args[1].as_str() {
        ...
        "keys-enter-recovery-mode" => bootstrap::cmd_keys_enter_recovery_mode(&args[2..]),
        "keys-export-identity" => bootstrap::cmd_keys_export_identity(&args[2..]),
        "keys-inspect" => bootstrap::cmd_keys_inspect(&args[2..]),
        "keys-mark-compromised" => bootstrap::cmd_keys_mark_compromised(&args[2..]),
        "keys-mark-revoked" => bootstrap::cmd_keys_mark_revoked(&args[2..]),
        "keys-rotate" => bootstrap::cmd_keys_rotate(&args[2..]),
