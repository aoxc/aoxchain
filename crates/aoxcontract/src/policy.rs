...
impl QuantumSecurityProfile {
    pub fn has_quantum_schedule(&self) -> bool {
        self.transition_epoch_start.is_some() || self.classical_retirement_epoch.is_some()
    }
}
...
impl Validate for ContractPolicy {
    fn validate(&self) -> Result<(), ContractError> {
        ...
        if self.quantum_security.migration_mode == QuantumMigrationMode::HybridDualSign
            && self.quantum_security.pq_signature_schemes.is_empty()
        {
            return Err(PolicyValidationError::PolicyViolation(
                "hybrid_dual_sign requires at least one pq signature scheme".into(),
            )
            .into());
        }
        if self.quantum_security.migration_mode == QuantumMigrationMode::HybridDualSign
            && self.quantum_security.min_signature_bundles < 2
        {
            return Err(PolicyValidationError::PolicyViolation(
                "hybrid_dual_sign requires min_signature_bundles >= 2".into(),
            )
            .into());
        }
        if self.quantum_security.migration_mode == QuantumMigrationMode::ClassicalOnly {
        ...
        if matches!(
            self.quantum_security.migration_mode,
            QuantumMigrationMode::HybridDualSign | QuantumMigrationMode::PostQuantumOnly
        ) && !self.quantum_security.has_quantum_schedule()
        {
            return Err(PolicyValidationError::PolicyViolation(
                "non-classical migration mode requires explicit transition schedule".into(),
            )
            .into());
        }
