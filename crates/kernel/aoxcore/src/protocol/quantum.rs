...
/// Kernel-scoped runtime wrapper for profile-bound quantum transaction handling.
#[derive(Debug)]
pub struct QuantumKernelRuntime {
    profile: QuantumKernelProfile,
    pool: crate::transaction::pool::QuantumTransactionPool,
}

/// Runtime failures while operating the quantum kernel path.
#[derive(Debug)]
pub enum QuantumKernelRuntimeError {
    InvalidProfile(QuantumProfileError),
    Pool(crate::transaction::pool::QuantumTransactionPoolError),
}

impl fmt::Display for QuantumKernelRuntimeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidProfile(err) => write!(f, "invalid quantum runtime profile: {err}"),
            Self::Pool(err) => write!(f, "quantum runtime pool operation failed: {err}"),
        }
    }
}

impl std::error::Error for QuantumKernelRuntimeError {}

impl QuantumKernelRuntime {
    pub fn new(profile: QuantumKernelProfile) -> Result<Self, QuantumKernelRuntimeError> {
        profile
            .validate()
            .map_err(QuantumKernelRuntimeError::InvalidProfile)?;
        Ok(Self {
            profile,
            pool: crate::transaction::pool::QuantumTransactionPool::new(),
        })
    }

    #[must_use]
    pub fn profile(&self) -> &QuantumKernelProfile {
        &self.profile
    }
    ...
    pub fn submit(
        &mut self,
        tx: crate::transaction::quantum::QuantumTransaction,
    ) -> Result<[u8; 32], QuantumKernelRuntimeError> {
        self.pool
            .add_with_profile(&self.profile, tx)
            .map_err(QuantumKernelRuntimeError::Pool)
    }

    pub fn build_block_tasks(
        &mut self,
        max_count: usize,
        max_payload_bytes: usize,
    ) -> Result<Vec<([u8; 32], crate::block::Task)>, QuantumKernelRuntimeError> {
        self.pool
            .drain_for_block(max_count, max_payload_bytes)
            .map_err(QuantumKernelRuntimeError::Pool)
    }
}
