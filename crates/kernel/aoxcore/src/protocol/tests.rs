...
#[test]
fn quantum_profile_forward_compatibility_requires_higher_version_and_default_support() {
    let current = QuantumKernelProfile::strict_default();

    let mut next = QuantumKernelProfile::strict_default();
    next.profile_version = 3;
    next.allowed_signatures = vec![SignatureScheme::MlDsa65, SignatureScheme::SphincsSha2128f];
    next.fallback_signature = Some(SignatureScheme::SphincsSha2128f);

    assert!(
        current
            .is_forward_compatible_with(&next)
            .expect("forward compatibility check must succeed")
    );

    let mut same_version = next.clone();
    same_version.profile_version = current.profile_version;
    assert!(
        !current
            .is_forward_compatible_with(&same_version)
            .expect("same version must not be forward compatible")
    );

    let mut incompatible = next;
    ...
    assert!(
        !current
            .is_forward_compatible_with(&incompatible)
            .expect("removing active default support must be incompatible")
    );
}
