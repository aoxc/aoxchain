...
## Controlled Evolution Without Kernel Drift

To keep the kernel stable while enabling improvement:

1. treat `KeyRealm`, `CryptoProfileId`, lane semantics, and continuity checks as compatibility-sensitive protocol contracts;
2. evolve only adapter/profile layers for new algorithms and witness encodings;
3. require governance ratification for any change that alters lane requirements or continuity validity rules;
4. keep fallback/deprecation windows explicit so profile upgrades do not cause unbounded policy drift.
