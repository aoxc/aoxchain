...
## Progress snapshot (as of 2026-04-01)
This section tracks implementation progress in coarse engineering percentages (not release-readiness percentages).

- Phase 1 — Transactional host journal and canonical state model: **70%**
  - implemented: transactional journal, checkpoints/rollback, lane-aware ops, conflict checks.
  - pending: explicit shared settlement state partitions and persistent journal backend abstraction.
- Phase 2 — Capability-based host security model: **65%**
  - implemented: capability profile types and host-operation gating.
  - pending: full per-lane policy governance integration and admission-time capability attestation.
- Phase 3 — Canonical receipt and proof surface: **60%**
  - implemented: canonical receipt projection, replay/trace/message roots.
  - pending: state-root delta proof contracts and receipt-level cryptographic compliance flags.
- Phase 4 — Cross-lane messaging protocol: **55%**
  - implemented: deterministic replay + causal ordering checks.
  - pending: async receipt references, delayed settlement queue, and message policy governance hooks.
- Phase 5 — Dependency-aware deterministic scheduler: **45%**
  - implemented: deterministic conflict-aware planning primitive from declared read/write sets.
  - pending: scheduler proof metadata and integration into block execution admission.
- Phase 6 — Determinism compliance contract: **20%**
  - implemented: baseline deterministic policy checks.
  - pending: explicit lane compliance contracts and exhaustive forbidden-nondeterminism enforcement.
- Phase 7 — Lane governance and classification: **15%**
  - implemented: early lane capability profile wiring.
  - pending: trust/isolation/upgrade policy descriptors and governance-controlled lane lifecycle.

Estimated overall kernel-hardening progress against this plan: **~47%**.
