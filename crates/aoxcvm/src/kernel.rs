...
pub enum KernelError {
    ...
    LanePolicyViolation(&'static str),
    CapabilityDenied {
        lane: LaneId,
...
mod tests {
    ...
    #[test]
    fn settlement_only_tx_requires_settlement_lane_class() {
        let mut lanes = LaneRegistry::default();
        lanes.register_with_config(
            LaneId::Core,
            CoreLane,
        ...
        let receipt = kernel.execute_tx(
            &sample_block(),
            &sample_tx_with_flags(
                LaneId::Core,
                200_000,
                IntentFlags::new(IntentFlags::SETTLEMENT_ONLY),
            ),
            &mut state,
        );
        ...
        assert_eq!(
            receipt.error,
            Some(KernelError::LanePolicyViolation(
                "settlement-only transactions require settlement lane class",
            ))
        );
    }
    ...
    #[test]
    fn untrusted_lane_cannot_submit_cross_lane_intent() {
        let mut lanes = LaneRegistry::default();
        lanes.register_with_config(
            LaneId::Core,
            CoreLane,
        ...
        let receipt = kernel.execute_tx(
            ...
            &sample_tx_with_flags(
                LaneId::Core,
                200_000,
                IntentFlags::new(IntentFlags::CROSS_LANE),
            ),
            &mut state,
        );
