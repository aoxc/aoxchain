...
### 2.6 Anahtar yönetimi (key management) dahil mi?

**Evet, bu bölüm anahtar yönetimini de kapsamalı.** Hatta güvenlik modelinin en kritik parçalarından biri budur.

Core-level key management kapsamı şunları içermeli:

- root seed / master secret sınırları,
- deterministic derivation domain'leri,
- role-based key ayrımı,
- encrypted at-rest saklama formatı,
- hot / warm / cold key ayrımı,
- operator approval ve rotation policy'si,
- revocation + recovery akışları,
- key compromise sonrası emergency prosedürü,
- audit log / evidence üretimi,
- backup, restore ve secure destruction kuralları.

Yani bu belge yalnızca "certificate doğrulama" belgesi değil; **node key lifecycle** belgesi olarak düşünülmelidir.
...
## 9. Bu bölüm tamam mı, eksik var mı?

Kısa cevap:

- **Mimari yön olarak güçlü bir temel olur.**
- **Güvenliği belirgin biçimde artırır.**
- **Ama tek başına 'tamamlandı' sayılmaz.**

Bu doküman şu anda bir **çekirdek güvenlik mimarisi ve kapsam tanımıdır**. Yani neyin core'da olması gerektiğini netleştirir. Fakat production-grade tamamlanmış sayılması için aşağıdaki implementasyon ve doğrulama katmanları da gerekir.

### 9.1 Henüz eksik kalacak şeyler

Aşağıdakiler yapılmadan "tamam" denmemeli:

- typed Rust struct implementasyonları,
- canonical serializer / deserializer yüzeyi,
- certificate-chain validation kodu,
- rotation continuity validator'ları,
- recovery authorization akışı,
- revocation persistence + distribution modeli,
- transport admission integration,
- consensus signer authorization integration,
- golden tests / adversarial tests / fuzzing,
- HSM/KMS/remote signer entegrasyon kararı.

### 9.2 Daha güvenli olur mu?

**Evet, ciddi şekilde daha güvenli olur.** Özellikle şu riskleri azaltır:

- tek anahtar sızıntısının tüm node'u düşürmesi,
- app-level bypass ile yetkisiz admission,
- yanlış certificate yorumlama,
- replay / stale rotation kabulü,
- validator kimliği ile transport kimliğinin karışması,
- recovery sürecinin kontrolsüz ilerlemesi.

Ama güvenlik artışı sadece mimari kararla değil, şu üç şeyle tamamlanır:

1. doğru implementasyon,
2. sert test matrisi,
3. bağımsız audit.

### 9.3 'Tamam' demek için kabul kriteri ne olmalı?

Bence bu bölüm ancak şu koşullarda tamam sayılmalı:

- `aoxcore::identity` içinde typed core model tamamlandıysa,
- key lifecycle policy kodla enforce ediliyorsa,
- CLI/app katmanı yalnızca wrapper rolündeyse,
- consensus ve transport aynı identity source of truth'u kullanıyorsa,
- rotation/revocation/recovery testleri geçtiyse,
- audit trail üretimi doğrulandıysa.

Bu seviyeye gelmeden "tamam" demek erken olur.

### 9.4 Anahtar yönetimi gerçekten dahil mi?

**Evet, dahil olmalı; hatta ayrı alt-program olarak yürütülmeli.**

Benim önerim key management backlog'unu şu başlıklara bölmek:
...
- key revocation
- key recovery
- key escrow policy (varsa)
- HSM/KMS integration
- remote signer policy
- operator approval thresholds
- secret destruction / decommissioning

Yani eğer anahtar yönetimi bu işin içine tam dahil edilmezse, güvenlik modelinin en kritik ayağı eksik kalır.

## 10. Mevcut entegrasyon durumu

Bu belgeye göre repoda artık şu bağlar oluşmuş durumda:

- `aoxcore::identity::key_bundle` ile typed canonical key bundle modeli var,
- `aoxcmd key-bootstrap` bu bundle'ı üretip saklıyor,
- `aoxcmd keys-verify` bundle + encrypted root seed doğrulaması yapıyor,
- `genesis-init` validator public key olarak canonical consensus key kullanabiliyor,
- `aoxc-keyforge key inspect-bundle` bundle JSON'unu denetleyebiliyor,
- `Block::validate_with_key_bundle()` producer alanını consensus key'e bağlayabiliyor.

Yani key sistemi artık sadece belge düzeyinde değil; çekirdek + CLI + bootstrap + block validation hattında kısmi gerçek entegrasyona geçmiş durumda.

## 11. Hâlâ kapanması gereken son boşluklar

Bu bölümün gerçekten "tam bitti" denmesi için aşağıdakiler de kapanmalı:

- consensus validator-set membership'in doğrudan bundle/certificate ile bağlanması,
- transport handshake tarafında transport key enforcement,
- rotation ve revocation'ın persistent/distributed hale gelmesi,
- recovery authorization ve emergency rollover akışları,
- remote signer / HSM / KMS desteği,
- node bootstrap sırasında certificate-chain ve passport projection üretiminin canonical bundle ile tam hizalanması.
