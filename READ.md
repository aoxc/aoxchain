# AOX Chain Workspace - Production Audit Guide
...
The repository is a Rust workspace that combines consensus, networking, execution, storage, SDK, operator tooling, mobile surfaces, RPC gateways, and AI-assisted governance support. The root folder is responsible for release discipline, dependency policy, and the evidence chain that makes every subcomponent reviewable.
...
- Define the canonical workspace members and the shared dependency baseline.
- Provide the master release label used by crates, binaries, configuration packs, and audit records.
- Capture the production-readiness story that security reviewers can follow without reverse-engineering the repository.

## Security Boundary and Audit Focus
At the root level, the most important controls are supply-chain discipline, reproducible builds, release metadata consistency, and evidence retention. Any mismatch between manifests, binary tags, and documentation is treated as an audit defect.
