...
- Aynı makinede izole testnet/mainnet temelini hızlıca kurmak için: `cargo run -q -p aoxcmd -- dual-profile-bootstrap --password <value> --output-dir ./stack`
- Bu komut `./stack/mainnet` ve `./stack/testnet` altında ayrı home dizinleri üretir ve port çakışmasını önleyen varsayılan port planını uygular.
- Sonraki adım olarak her iki home için `mainnet-readiness --write-report` çalıştırılıp ayrı kapanış raporları alınmalıdır.
